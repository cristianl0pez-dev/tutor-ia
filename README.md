# Tutor IA Pedagógico — Python para Principiantes

## ¿Qué es esto?

Un sistema de tutoría de programación que te enseña Python de forma guiada, progresiva y sin que simplemente copies soluciones.

## ¿Cómo funciona?

### Comandos Disponibles

| Comando | Para qué sirve |
|---------|---------------|
| `/teach <tema>` | Aprender un concepto nuevo |
| `/practice <tema>` | Practicar lo aprendido |
| `/review` | Revisar tu trabajo |
| `/next` | Avanzar al siguiente nivel |
| `/hint` | Pedir una pista |
| `/simplify <tema>` | Pedir una explicación más simple |

### Flujo de Aprendizaje

1. **Diagnóstico** → Te preguntamos qué sabes
2. **Enseñanza** → Aprendes el concepto con ejemplos
3. **Práctica** → Ejercicios de dificultad progresiva
4. **Validación** → Explicas tu razonamiento
5. **Feedback** → Recibes retroalimentación
6. **Desbloqueo** → Avanzas cuando estás listo

## Módulos

| Módulo | Tema | Estado |
|--------|------|--------|
| 01 | Variables y tipos de datos | Disponible |
| 02 | Condicionales | Bloqueado |
| 03 | Loops (bucles) | Bloqueado |
| 04 | Funciones | Bloqueado |

## Para Empezar

Escribe en el chat:

```
/teach variables
```

Y comenzamos.

## Reglas del Sistema

- No se entregan soluciones completas
- Debes explicar tu razonamiento
- El avance depende de tu comprensión
- Puedes pedir pistas sin problema
- Si no entiendes, pide `/simplify`

## Tu Progreso

Tu progreso se guarda automáticamente en `progreso/student_progress.md`.
